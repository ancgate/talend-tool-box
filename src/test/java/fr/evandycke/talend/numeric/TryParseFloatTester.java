...
/**
 * TryParseFloatTester est la classe de test de conversion d'un objet en Float.
 * 
 * @author elie.vandycke
 * 
 */
public class TryParseFloatTester extends TestCase {
	...
	public void setUp() throws Exception {
		...
		if (this.list.isEmpty()) {
			...
			this.list.add(new TestSet("100", new Float(100)));
			this.list.add(new TestSet("1000", new Float(1000)));
			this.list.add(new TestSet("27,5", new Float("27.5")));
			this.list.add(new TestSet("27.5", new Float("27.5")));
	...
	public final void testTryParse() {
		...
		for (TestSet t : this.list) {
			Float result = FloatHelper.tryParse(t.getValue());
