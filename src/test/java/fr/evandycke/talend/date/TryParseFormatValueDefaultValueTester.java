 ...
 * 
 */
public class TryParseFormatValueDefaultValueTester extends TestCase {
	...
	/**
	 * Modèle de date par défaut
	 */
	private static final String DEFAULT_PATTERN = "dd/MM/yy";
	...
	public void setUp() {
		...
		if (list.isEmpty()) {
			...
			list.add(new TestSet(DateHelper.getDate(2007, 1, 4), DateHelper
					.getDate(2007, 1, 4)));
		}
	}
	...
	public void testTryParse() {
		...
		for (TestSet t : list) {
			...
			Date result = DateHelper.tryParseFormat(DEFAULT_PATTERN, t.getValue(),
					DEFAULT_VALUE);
	 ...
	 */
	@Test
	public void testTryParseWithNullPattern() {

		for (TestSet t : list) {

			Date result = DateHelper
					.tryParseFormat(null, t.getValue(), DEFAULT_VALUE);
			log.debug("Valeur : %s - Attendu : %s - Trouvé : %s", t.getValue(), t.getExpectedResult(),
					result);
			assertEquals(t.getExpectedResult(), result);
		}
	 ...
	 */
	@Test
	public void testTryParseWithEmptyPattern() {

		for (TestSet t : list) {

			Date result = DateHelper.tryParseFormat("", t.getValue(), DEFAULT_VALUE);
			log.debug("Valeur : %s - Attendu : %s - Trouvé : %s", t.getValue(), t.getExpectedResult(),
					result);
			assertEquals(t.getExpectedResult(), result);
		}
